...
function guessFromIngredients(ingredientNames: string[]): StepGuess {
  ...
  if (filter) {
    return {
      stepType: 'spf',
      source: 'ingredients',
      reason: `contains the UV filter ${filter.inci_name}`,
    };
  }
  ...
  if (acid) {
    return {
      stepType: 'exfoliant',
      source: 'ingredients',
      reason: `contains ${acid.inci_name}`,
    };
  }
  ...
  if (richCount >= 3 && richCount / rows.length >= 0.3) {
    return {
      ...
      source: 'ingredients',
      reason: 'rich in occlusives and emollients',
    };
  }
  ...
  if (active) {
    return {
      stepType: 'serum',
      source: 'ingredients',
      reason: `built around ${active.inci_name}`,
    };
  }
...
export function guessStepFromName(name: string): StepGuess {
  ...
  for (const rule of NAME_RULES) {
    ...
    if (hit) {
      return { stepType: rule.step, source: 'name', reason: `matched “${hit.trim()}”` };
    }
  }
